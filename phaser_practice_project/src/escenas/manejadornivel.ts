...
export default class ManejadorNivel extends Phaser.Scene 
{
    ...
    creaBandasonora(): void{
        ...
        if (mibd.datos.musica){
            ...
            this.bandasonoraNivel = this.sound.add(Constantes.SONIDOS.BANDASONORA + this.nombreNivel , {loop:true, volume:0});
    ...
    creaMapaNivel(jsonMapa: string, imagenMapa: string = Constantes.MAPAS.TILESET): void {
          ...
          //capa de plataformas
          this.capaPlataformasMapaNivel = this.mapaNivel.createLayer(Constantes.MAPAS.CAPA_PLATAFORMAS, this.conjuntoPatrones);
    ...
    /**
     * Finaliza nivel  parando música, y las dos escenas HUD y la del Nivel
    ...
    finalizaNivel(esWin: boolean = true): void{          
        ...
        this.scene.start(Constantes.ESCENAS.FINNIVEL, {
            ...
            puntuacion: this.puntuacion + this.tiempoRestante + (this.vidas * 100)
        });
    ...
    */
    creaRecolectables(recolectablesConfig: any[]): void{
        recolectablesConfig.forEach(enemigosConfig => {
            let recolectables: any  = new Recolectables(this,Constantes.MAPAS.RECOLECTABLES, enemigosConfig.ID, enemigosConfig.ANIM);
